Not: Eğer türkçe karakterler görünmüyorsa ":set encoding=utf-8" yazarak düzeltebilirsiniz(vimrc'ye de eklenebilir).

İçindekiler:
(Arama esnasında büyük küçük duyarlılığı varsa hepsini küçük yazın)

1- Yazma Moduna Geçiş
2- Dosya İçinde Hareket
3- Silme, Kopyalama ve Yapıştırma
4- Dosya veya Shell'den Veri Alma
5- Depo kullanarak kopyalama ve Yapıştırma
6- Görsel Modda Kopyalama ve Yapıştırma
7- Kesme ve Değiştirme
8- Arama ve Otomatik Değiştirme
9- Undo, Redo
10- Biçimlendirme
11- Yazma/İşlem Kolaylaştırıcılar
12- Macro
13- Dosya İşlemleri
14- Birçok Doya İle Aynı Anda Çalışma
15- Tab'lar İle Çalışma  
16- Birçok Pencere İle Aynı Anda Çalışma
17- Shell
18- Vim'i açarken Komut Çalıştırma
19- Faydalı Linkler
...
zt          bulunulan satırı sayfanın başında gösterir(imlecin yeri değişmez) (t=top)
...
#cw          # kelimeyi sil ve yerine, yazılanları yaz
...
:s/sablon/yeni/g    satırda, sablona uyan tüm metinleri, yeni ile değiştir (g=global)
:%s/sablon/yeni/g   dosyada, sablona uyan tüm metinleri, yeni ile değiştir (%s dosya)
:%s/sablon/yeni/gc  dosyada, sablona uyan metinleri, yeni ile değiştir ama önce onay alır (c=confirm)
şablonda ^      satır başı
şablonda $      satır sonu
...
* add comment/uncomment multiple lines 
      
   1- ilk olarak esc ile command mode'dan çıkıyoruz.
   2- Ctrl+V ile visual mode'a geçiyoruz.
   3- ok tuşları veya j/k tuşları ile çalışcağımız satırları seçiyoruz.
   4- I tulu ile intsert moda  geçiyoruz.
   5- insert moda'a geçtikten sonra # comment ekliyoruz.esc ile command mode'dan çıkıyoruz.
   
   diğer bir yöntem ise vim içersinde :%s/^/# çalıştırmak.
   
* delete comment/uncomment multiple lines   
   
    1- ilk olarak esc ile command mode'dan çıkıyoruz.
    2- Ctrl+V ile visual mode'a geçiyoruz.
    3- ok tuşları veya j/k tuşları ile çalışcağımız satırları seçiyoruz.
    4- x tuşu ile seçili satırları siliyoruz.
